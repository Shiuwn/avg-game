...
function initNPCs() {
  ...
  let maleNPC = getMaleNPC(npcs)
  let femaleNPC = getFemaleNPC(npcs)
  const giveAge = (count, cb) => {
    each(discard(maleNPC, count), (_, npc) => {
      cb(npc)
    })
    each(discard(femaleNPC, countNPC * 0.07), (_, npc) => {
      cb(npc)
    })
  }
  // FIXME: 性别赋值可以优化，考虑放在生成的迭代中
  // FIXME: 设置一个缓存窗口在缓存窗口中设置值
  // 性别赋值
  // <14 为儿童 男女各占 7%
  giveAge(countNPC * 0.07, (npc) => {
    npc.年龄 = random({ min: 0, max: 13 })
    npc.职业 = '赋闲'
  })

  // 14<= 年龄<18 男女各占10% 青少年

  giveAge(countNPC * 0.1, (npc) => {
    npc.年龄 = random({ min: 14, max: 17 })
  })
  // 18<= 年龄 <60 成年人 占比 30%
  giveAge(countNPC * 0.3, (/**@type {NPC} */ npc) => {
    npc.年龄 = random({ min: 18, max: 59 })
    npc.职业 = '赋闲'
  })
  // >=60 老年人 各占比 3%
  giveAge(countNPC * 0.03, (npc) => {
    npc.年龄 = random({ min: 60, max: 70 })
  })
